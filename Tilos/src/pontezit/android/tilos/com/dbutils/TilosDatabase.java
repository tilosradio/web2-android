...
package pontezit.android.tilos.com.dbutils;
...
public class TilosDatabase extends SQLiteOpenHelper {
	public final static String TAG = TilosDatabase.class.getName();
	...
	public static final String	DATABASE_NAME = "tilos.db";
    ...
    public TilosDatabase(Context context) {
		...
		switch (oldVersion) {
			...
			case 4:
				...
				for (int i = 0; i < uris.size(); i++) {
					...
					values.clear();
					values.put(TilosDatabase.FIELD_STREAM_LIST_POSITION, uri.getId());
	...
	public UriBean saveUri(UriBean uri) {
		...
		synchronized (dbLock) {
			...
			ContentValues values = new ContentValues();
			values.put(TilosDatabase.FIELD_STREAM_LIST_POSITION, id);
